...
### 프로그램 개요
- 프로그램이란 : 컴퓨팅 환경(PC, 모바일, 임베디드 등)에서 `사용자의 요구`에 따라 `데이터를 처리`(표현, 이동, 저장, 로드 등)할 수 있는 `소프트웨어`
- 일의 순서도 의미함. 일의 순서를 컴퓨팅 환경 내에서 실행하는 것
...
### 컴파일과 컴파일러
- 컴파일(빌드) : 소스코드 파일(텍스트 파일)을 컴퓨터가 실행할 수 있는 파일로 변경, 생성
    - 전처리, 오브젝트화, 링킹을 통해 실행파일로 만드는 과정
    - c, cpp -> 확장자 exe 파일로 생성
- 컴파일러 : IDE(Integrity Development Environment) 툴을 사용
    - IDE : `Visual Studio`, Visual Studio Code, Rider, Eclipse 등
...
8. 프로젝트가 여러개일 때 시작 프로그램 변경
    - `솔루션 속성` > `시작 프로젝트 구성` > `현재 선택 영역`으로 선택

    ![alt text](image-11.png)

    - 포커스 중인 프로젝트가 시작프로젝트로 설정됨

### C 기본 문법

1. 기본 사용법

    - 메인 함수 작성법 : [확인](./Day01/Chap02-Solution/Chap02-01-app/main.c)
...
1. 기본 문법
    - 데이터 형 : [확인](./Day02/Chap03-Solution/Chap03-01-app/main.c)
    ...
    - 연산자 : [확인](./Day02/Chap04-Solution/Chap04-01-app/main.c)
        - 수치(산술) 연산, 대입 연산, 논리 연산, 관계 연산, 비트 연산 등
...
2. 반복문
    ...
    - break : [확인](./Day03/Chap06-Solution/Chap06-04-app/main.c)
    - 무한루프, continue, break : [확인](./Day03/Chap06-Solution/Chap06-05-app/main.c)
...
1. 함수 계속
    - 함수 유형 : [확인](./Day03/Chap07-Solution/Chap07-02-app/main.c)
        ...
        - 매개변수와 반환값이 있는 함수 - Chap07-01-app 참고
...
1. **포인터**
    ...
    - 포인터 이해 : [확인](./Day05/Chap09-Solution/Chap09-04-app/main.c)
        - 변수의 주소(메모리상)를 담고, 간접참조연산자(*)로 그 변수의 값을 사용하기 위한 것
        - 변수의 주소를 변경해서 `여러 변수를 사용가능`, 함수도 주소를 변경해서 여러 함수를 사용가능
        - 변수의 주소는 바뀌지 않는 값&(상수). 변수의 주소는 바꿀 수 없다.
        - 변수의 가리키는 값(*)은 변경 가능(변수)
        - 포인터에 주소를 저장할 때는 자료형이 동일해야 한다. int -> int*

    - 포인터 크기 : [확인](./Day05/Chap09-Solution/Chap09-05-app/main.c)
        - 주소의 크기와 동일 - 8bytes(64bit이므로)
    
    - 포인터 사용 이유 : [확인](./Day05/Chap09-Solution/Chap09-07-app/main.c)
        ...
        - 함부로 변수를 많이 찍어서 만들 수 없음(용량문제 발생)!
        - 주소를 공유해서 함수 어디서나 변수의 값을 변경할 수 있음
        - 함수 간에 효과적으로 데이터를 공유하고 용량(메모리, 저장소)을 절약!!

2. 배열과 포인터 : [확인](./Day05/Chap10-Solution/Chap10-01-app/main.c)
    - 배열과 포인터는 매우 흡사해서 유사하게 사용가능
    - scanf() 사용시 배열 str[80] 선언 후, str(배열의 이름자체가 주소임 == &str[0])로 사용
    - 직접적인 연관이 있음
    - 실무에서 데이터를 공유하면 보통 배열도 많이 사용됨
    - 배열을 그대로 매개변수로 전달하면 큰 메모리를 많이 사용하게 됨
    - int ary[100] -> main에서 400bytes, 함수에서 400bytes의 메모리 공간 확보(`메모리 부족사태`)
...
4. 문자열 기본 : [확인](./Day05/Chap12-Solution/Chap12-01-app/main.c)
    - 문자열 상수와 문자 포인터변수 사용법

## 6일차 (02.09)

### C 중급 문법

4. 문자열과 포인터 : [확인](./Day06/Chap12-Solution/Chap12-02-app/main.c)
    - 문자열 연산 함수 : 자주 사용하는 문자열 처리 내장함수
        - `strlen()` : 문자열 길이 리턴함수
        - `stcpy()`, strncpy() : 문자열 대입함수. n은 글자갯수만큼 대입
        - strcat(), strncat() : 문자열 합치기 함수
        - `strcmp()`, strncmp() : 문자열 비교함수. 두 문자열이 같은지 확인
        - strcspn() : 문자열에서 찾고자 하는 문자열에 포함된 문자를 검색. 일치할 때까지 읽어들인 문자의 갯수 리턴
        - strtok() : 패턴 문자열로 기존 문자열을 자르는 함수
        - strchr() : 문자열에서 특정 문자가 가장 먼저 나타나는 곳의 위치를 리턴하는 함수

5. Tip
    - #pragma region, #pragma endregion : 소스코드 영역에서 일부내용을 축소(확장)시키고자 할 때

6. 변수 영역 : [확인](./Day06/Chap13-Solution/Chap13-01-app/main.c)
    - 같은 이름의 변수가 있으면, 코드 블록 내의 변수가 우선적으로 사용됨!!
    - `auto` : 지역. auto라는 키워드를 사용하지 않음.
    - global : 전역. ~~global~~ 단어를 사용하는게 아님. 전역 변수는 문제가 많아 제한적으로 사용할 것.
    - `static` : 정적. 소스코드 전체가 실행되는 동안 살아있는 변수. [확인](./Day06/Chap13-Solution/Chap13-02-app/main.c)
    - `register` : CPU내 존재하는 메모리를 뜻함. 데이터 처리속도가 가장 빠름. 거의 쓸일 없음.
...
1. 함수 포인터 : [확인](./Day07/Chap15-Solution/Chap15-03-app/main.c)
    - 함수도 메모리 상에 주소(&)를 가지고 있음.
    - 변수와 동일하게 함수도 주소를 통해서 여러 함수를 변경해서 사용.
    - `어떤 함수를 연결하느냐에 따라 동작이 바뀐다.`
    - 실행 시점에 어떤 함수가 실행될지 모를 때 사용 : [확인](./Day07/Chap15-Solution/Chap15-04-app/main.c)
    - 조건문을 줄이고, 구조를 유연하게 만들기 위해서 사용.
    - 사용 예 : 임베디드 하드웨어 이벤트 인터럽트, 상태머신, GUI 이벤트, 게임 상태별 로직

2. void 포인터 : [확인](./Day07/Chap15-Solution/Chap15-05-app/main.c)
...
3. 메모리 동적할당
    ...
    - `malloc` : memory acllocation. 메모리 동적 할당 : [확인](./Day07/Chap16-Solution/Chap16-01-app/main.c)
    - `calloc` : contiguos allocation. 연속된 메모리 동적 할당. 0으로 초기화. malloc보다 느림 : [확인](./Day07/Chap16-Solution/Chap16-03-app/main.c)
...
4. 프로그램의 메모리 영역
    ...
    - `코드 영역` : 소스코드가 실행하는 영역
    - `스택` : 지역변수 영역
    - `힙` : 동적할당 영역, Pointer, NULL 등
    - `기타 데이터 영역` : 전역변수, static변수 영역

5. 동적 할당 저장공간 활용 : [확인](./Day07/Chap16-Solution/Chap16-04-app/main.c)
    - 영어 사전과 같은 데이터를 저장할 때 : [확인](./Day07/Chap16-Solution/Chap16-05-app/main.c)
        - 배열로 처리하면 메모리 낭비가 심함
        - 동적할당으로 메모리 절약
...
7. 사용자 정의 자료형 - 구조체 : [확인](./Day07/Chap17-Solution/Chap17-01-app/main.c)
    - C언어는 절차적 프로그래밍 언어
    - C++는 객체지향 프로그래밍 언어, C#, Java, Python 등
...
1. 사용자 정의 자료형 - 구조체 : [확인](./Day08/Chap17-Solution/Chap17-02-app/main.c)
    - 구조체 사용 : [확인](./Day08/Chap17-Solution/Chap17-03-app/main.c)
    - 구조체 내 구조체 : [확인](./Day08/Chap17-Solution/Chap17-04-app/main.c)
    - 구조체 포인터 : [확인](./Day08/Chap17-Solution/Chap17-05-app/main.c)

2. 사용자 정의 자료형 - 공용체, 열거형 : [확인](./Day08/Chap17-Solution/Chap17-06-app/main.c)

3. 파일 입출력
    - 파일 종류 : 텍스트 파일, 바이너리(2진) 파일
    - 바이너리 파일 : 데이터 파일, 내부적으로 0과 1로 저장된 파일. dat, exe, mp3, mp4, hwp, doc...
...
2. 폴더 및 파일경로
    - 절대 경로 : `C:\DEV\IDE\Microsoft VS Code\591199df40\resources\app\LICENSE.rtf` 와 같이, 드라이브부터 전체 경로를 모두 기재
        - 장점 : 정확한 경로가 표시
        - 단점 : 경로 문자열이 아주 길어질 수 있음

    - 상대 경로 : `.\LICENSE.rtf` 와 같이, .(현재 폴더), ..(부모 폴더) 기호를 사용해서 짧게 경로를 표시 방법
        - 장점 : 경로를 짧게 표시
        - 단점 : 개발자 직접 경로가 어떤식으로 구성되는 파악 필요. 같은 폴더에 존재하는 경우 사용 가능

3. 파일 입출력 : [확인](./Day09/Chap18-Solution/Chap18-02-app/main.c)
    - 키보드 등 입력은 버퍼를 통한 입력 스트림(stream)에, 출력은 출력 스트림(stream)에 보관되어 있음.
    - 파일, 콘솔 등에 전달

    ![alt text](image-15.png)

    - fgetc() : 파일에서 글자 하나씩 읽는 함수. 파일의 마지막(EOF - End Of File)까지 하나씩 읽는 함수
    - fputc() : 파일의 글자를 한자씩 출력(쓰는)하는 함수
    - fseek(), rewind() : 파일의 처음으로 돌아가서 다시 읽기
    - feof() : 파일의 끝까지 다 읽었는지 확인하는 함수
    - `fgets()` : 파일의 커서(현재 위치)에서 한 줄을 읽는 함수
    - `fputs()` : 파일의 한 줄을 출력(쓰는)하는 함수
    - fscanf() : 포맷에 맞춰서 파일을 읽는 함수
    - fprintf() : 포맷에 맞춰서 파일에 쓰는 함수
    - stdio.h : stdin(표준 입력), stdout(표준 출력), stderr(표준 에러) 포함
    - 인코딩 방식 : `EUC-KR`(CP-949, 멀티바이트, 한글만 표현, 2byte), `UTF-8`(한 글자 최대 6byte, 한글은 3byte)

4. 파일 입출력 모드 - 바이너리, 텍스트 : [확인](./Day09/Chap18-Solution/Chap18-06-app/main.c)
    - r : 읽기(read) 모드
    - w : 쓰기(write) 모드
    - a : 추가(append) 모드
    - t : 텍스트(text) 모드(생략 가능)
...
5. 한글 깨짐 현상 : [확인](./Day09/Chap18-Solution/Chap18-03-app/main.c)
...
6. 파일 핸들링 현상
    ...
    - fseek() 함수 사용시 : [확인](./Day09/Chap18-Solution/Chap18-04-app/main.c)
        - SEEK_SET : 파일의 최초 위치
        - SEEK_CUR : 파일의 현재 위치
        - SEEK_END : 파일의 마지막 위치

7. 전처리 지시자 : [확인](./Day09/Chap19-Solution/Chap19-01-app/main.c)
    - #include : 파일 포함 지시자. 내장 헤더파일은 <>로, 사용자 헤더파일은 "" 사용
    - #define : 새로 정의하는 지시자, 자주 사용될 상수 값을 정의할 때 많이 사용. 주로 대문자로 지정
...
8. 분할 컴파일 : [확인](./Day09/Chap19-Solution/Chap19-04-app/main.c)
    ...
    - 전역변수 static : 자신의 소스코드 외부에 공개하지 않는 내부용 정적 변수
...
1. static (**19장 내용 정정**)
    - 정적 키워드는 소스코드가 동작하는 동안 끝까지 살아남는 변수, 함수
    - static으로 선언하면 자기 소스 내에서는 위와 같이 동작
    - 외부 소스로 공유하지 않음 <-> `객체지향 언어와 정반대`!
    - 외부 소스에 공개하지 않을 변수를 static으로 지정할 것
    - 두 소스에 같은 이름의 함수나 변수를 쓰는데 중복 문제를 해결하기 위해

2. 토이프로젝트 - [주소록 프로그램](./TOYPROJECT.md)
