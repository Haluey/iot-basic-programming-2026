...
### 프로그램 개요
- 프로그램이란 : `사용자의 요구`에 따라 `데이터를 처리`(표현, 이동, 저장, 로드 등)할 수 있는 `소프트웨어`
- 일의 순서를 의미함. 일의 순서를 컴퓨터 내에서 실행하는 것
...
### 컴파일과 컴파일러
- 컴파일 : 소스코드 파일(텍스트 파일)을 컴퓨터가 실행할 수 있는 파일로 변경, 생성
    - 전처리, 링킹, 오브젝트화, 실행파일로...
    - c, cpp -> 확장자 exe 파일로 생성
- 컴파일러 : IDE(Integrity Development Environment) 툴을 사용
    - `Visual Studio`, Visual Studio Code, Rider, Eclipse 등
