...
int main(void) {
	char cwd[256];		// 경로 입력 변수, 윈도우 경로 문자열 최대 길이. Python 설치 시 제한 해제 가능한 거 확인 가능
	...
	// 여기서 부터 텍스트 파일 복사
	FILE* fp, * ofp;	// fp 파일 입력용 파일포인터, ofp 출력용 파일포인터
	
	// 디렉토리, 폴더 구분자로 "/"를 사용해도 무방
	fp = fopen("./data/sample.txt", "r");
	if (fp == NULL) {	// 실패 방지 옵션
		printf("파일 열기 실패\n");
		return 1;		// exit(1)과 동일한 기능. 완전히 동일하지는 않음
	}
	else {
		printf("파일 열기 성공\n");
	}
