...
static void sort_by_name(void);		// 이름순 정렬 qsot() 함수 사용, stdlib.h에 포함
// 매개변수는 void 포인터
...
int main(void) {
	...
	if (!ab_init()) {
		return 1;	// main함수의 return 1과 사용자 함수들의 return 1의 차이 비교 이해할 것!
	}
...
static void search_contact(void) {
	...
	for (i = 0; i < count; i++) {  // 실제 인덱스 0, 1, 2. 근데 순번에서는 1, 2, 3
...
static void update_contact(void) {
	...
	if (buf[0] != '\0') {  // 엔터만 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].phone[PHONE_LEN - 1] = '\0';
	}
...
static int load_contacts(const char* filename) {
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		//if (count >= MAX_CONTACTS) break;		// 현재 100개밖에 못넣음
		if (!ab_ensure_capacity(count + 1)) {
			puts("불러오기 중 메모리 부족!");
			break;
		}
		...
		contacts[count].phone[PHONE_LEN - 1] = '\0';	// 전화번호 길이가 너무 길면 정리
