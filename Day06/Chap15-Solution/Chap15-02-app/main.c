...
void swap_ptr(char**, char**);	// 포인터 값을 바꾸는 함수의 매개변수
void print_str(char**, int);	// 포인터 배열을 매개변수로 받는 함수
...
int main(void) {
	...
	char* ptr_animal[] = { "eagle","tiger","lion","squirrel" };		// char** ptr_animal = { "eagle","tiger","lion","squirrel" }; 이건 불가능
	// 이중 포인터는 대입이나 매개변수로 전달할 때 가능
	char** ptr = ptr_animal;
